...
/**
 * Diagrama de ecosistema tipo "Orbiting Circles" (patrón Magic UI), en estático.
 * Crisopa en el centro; ChatGPT y Claude orbitan en el anillo interior
 * (las puertas hacia la IA) y los conectores en el exterior. Cada nodo se
 * coloca en su ángulo con CSS (`.orbit-node` en global.css); no gira, porque
 * la guía visual no quiere nada que se mueva por decorar. No necesita
 * hidratación ni envía JS al cliente.
 */
...
interface OrbitRingProps {
  ...
  radius: string
  size: string
  /** Giro inicial del anillo, para que sus nodos no se alineen con los del otro. */
  offset?: number
}

function OrbitRing({ nodes, radius, size, offset = 0 }: OrbitRingProps) {
  return (
    <>
      <div
        aria-hidden="true"
        className="absolute left-1/2 top-1/2 rounded-full border border-dashed border-line-strong"
        style={{
          width: `calc(${radius} * 2)`,
      ...
      />
      {nodes.map((item, i) => {
        const angle = offset + (360 / nodes.length) * i
        return (
          <div
            key={item.label}
...
const connectorTile =
  'flex h-full w-full items-center justify-center rounded-md border border-line-strong bg-surface text-foreground-muted shadow-1'
...
const connectors: OrbitNode[] = [
  ...
  },
  {
    ...
    node: (
      <div className={connectorTile}>
        <span className="font-mono text-[clamp(7px,2.6cqw,11px)] font-medium tracking-tight text-foreground-muted">
          MAPA
        </span>
      </div>
    ),
  },
  {
    ...
    node: (
      <div className={connectorTile}>
        <span className="font-mono text-[clamp(8px,2.8cqw,12px)] font-medium text-foreground-muted">
          +más
        </span>
      </div>
    ),
...
const aiNodes: OrbitNode[] = [
  {
    label: 'ChatGPT',
    node: (
      <div className="flex h-full w-full items-center justify-center rounded-md bg-[#0e1117] text-white shadow-1">
...
export default function OrbitingConnectors() {
  return (
    ...
    >
      {/* Conectores (exterior) */}
      <OrbitRing nodes={connectors} radius="40cqw" size="min(48px,11cqw)" offset={22.5} />

      {/* IA (interior) */}
      <OrbitRing nodes={aiNodes} radius="25cqw" size="min(56px,14cqw)" offset={90} />

      {/* Crisopa (hub central) */}
      <div
        className="relative z-10 flex items-center justify-center rounded-lg bg-primary-600 text-white shadow-2"
